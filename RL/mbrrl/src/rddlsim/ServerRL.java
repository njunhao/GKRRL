/**
 * RDDL: Main server code for interaction with RDDLSim client - modified to work with RL experiments
 * 
 * @author Alvin
 * @version 2019
 *
...
public class ServerRL implements Runnable {
	...
	public static void main(String[] args) {
		...
		try {
			...
			while (true) {
				...
				Runnable runnable = new ServerRL(connection, ++ID, rddl, state_viz, port, rdg);
	...
	ServerRL (Socket s, int i, RDDL rddl, StateViz state_viz, int port, RandomDataGenerator rgen) {
		this.connection = s;
		this.id = i;
		this.rddl = rddl;
			...
			for( ; r < numRounds && !OUT_OF_TIME; r++ ) {
				...
				while (true) {
					...
					msg = createXMLTurn(state, h+1, domain, observStore, 
							...
							immediate_reward, action_executed);
...
//						break;
					...
					if (action_executed) {  // don't compute next state if action fails to execute, assume state unchanged
						try {
...
//					System.out.println(domain.toString());
					...
					if (action_executed) {
						state.advanceNextState();   // this will change the state, if action did not executed, do not call!
					}
				...
				msg = createXMLRoundEnd(requestedInstance, r, accum_reward, h,
							timeAllowed - System.currentTimeMillis() + start_time,
                            clientName, immediate_reward, state, domain, observStore, action_executed);
				if (SHOW_MSG)
					System.out.println("Sending msg:\n" + msg);
				sendOneMessage(osw, msg);
			...
			msg = createXMLSessionEnd(requestedInstance, accum_total_reward, r+1,
...
//		System.out.println(osw.getEncoding());
	...
	static String createXMLSessionInit (int numRounds, double timeAllowed, ServerRL server) {
		DocumentBuilderFactory dbf = DocumentBuilderFactory.newInstance();
		try {
			DocumentBuilder db = dbf.newDocumentBuilder();
			Document dom = db.newDocument();
...
//			System.out.print(Client.serialize(dom));
			return Client.serialize(dom);
		}
		catch (Exception e) {
			System.out.println(e);
	...
	static boolean processXMLRoundRequest (DOMParser p, InputSource isrc,
        ...
        try {
			...
			throw new IllegalArgumentException("Illegal message from server: " + e.getNodeName());
		} catch (SAXException e1) {
			// TODO Auto-generated catch block
			e1.printStackTrace();
		} catch (IOException e1) {
...
//		NodeList nll = ele.getElementsByTagName("*");
	...
	static String createXMLTurn (State state, int turn, DOMAIN domain,
                                     ...
                                     double timeLeft, double immediateReward, 
                                     boolean action_executed) throws Exception {
		DocumentBuilderFactory dbf = DocumentBuilderFactory.newInstance();
		try {
			DocumentBuilder db = dbf.newDocumentBuilder();
			Document dom = db.newDocument();
			...
			// alvin addition
			if (turn > 1) {  // skip first iteration
				Element exStatus = dom.createElement(EXECUTION_STATUS);
				if (action_executed) {
					Text exStatusText = dom.createTextNode("success");
					exStatus.appendChild(exStatusText);
				}
				else {
					Text exStatusText = dom.createTextNode("failure");
					exStatus.appendChild(exStatusText);
				}
				rootEle.appendChild(exStatus);
			}
	...
	static String createXMLRoundEnd (String requested_instance, int round, double reward,
		...
		try {
            ...
            if (action_executed) {
            	addOneText(dom,rootEle, EXECUTION_STATUS, "success");
            }
            else {
            	addOneText(dom,rootEle, EXECUTION_STATUS, "failure");
            }
